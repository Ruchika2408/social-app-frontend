...
const resetPassword = async (email, oldPassword, newPassword) => {
    try {
        ...
        const response = await axios.post("http://localhost:3001/api/auth/resetpassword", { email, oldPassword: oldCiphertext, newPassword: ciphertext }).then((response) => response.data);
